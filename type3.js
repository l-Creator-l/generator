export default function type3(json, yMirror, returnData, additionalData) {
	...
	const walls = {
		...
		setWalls() {
			...
			if (wallsTMP.horizontal[0].coordinates.first[1] < wallsTMP.horizontal[1].coordinates.first[1]) {
				this.topWall = {
					x: 100,
					y: 100,
			...
			if (this.topWall.length !== this.bottomWall.length) {
			...
			if (wallsTMP.vertical[0].coordinates.first[0] < wallsTMP.vertical[1].coordinates.first[0]) {
				this.leftWall = {
					x: 100,
					y: 100,
			...
			if (this.leftWall.length !== this.rightWall.length) {
	...
	const container = {
		setContainer() {
			this.width = walls.topWall.length + 200;
			this.height = walls.leftWall.length + 200;
		}
	}
	...
	const wallPanels = {
		...
		setWallPanels() {
			...
			switch (walls.topWall.holes.length) {
			...
			switch (walls.bottomWall.holes.length) {
			...
			switch (walls.rightWall.holes.length) {
			...
			switch (walls.leftWall.holes.length) {
		...
		countWallPanels() {
			...
			outerPanelsCounter(this.topWall);
			outerPanelsCounter(this.bottomWall);
			outerPanelsCounter(this.leftWall);
			outerPanelsCounter(this.rightWall);
	...
	const svg = `
		<svg class="document__svg" width="${container.width}" height="${container.height}" style="transform: scale(${yMirror ? ("1, -1") : 1})" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
			...
			<rect x="${racks.topLeft.x}" y="${racks.topLeft.y}" width="${racks.size}" height="${racks.size}" fill="white" stroke="black"/>
			<rect x="${racks.bottomLeft.x}" y="${racks.bottomLeft.y}" width="${racks.size}" height="${racks.size}" fill="white" stroke="black"/>
			<rect x="${racks.topRight.x}" y="${racks.topRight.y}" width="${racks.size}" height="${racks.size}" fill="white" stroke="black"/>
			<rect x="${racks.bottomRight.x}" y="${racks.bottomRight.y}" width="${racks.size}" height="${racks.size}" fill="white" stroke="black"/>

			${(wallPanels.topWall.map(panel => `<rect x="${panel.start.x}" y="${panel.start.y}" width="${panel.size}" height="${walls.thickness}" fill="${panel.size >= 30 ? 'rgb(202, 248, 201)' : 'rgb(250, 127, 137)'}" stroke="black"/>`)).join('')}
			${(wallPanels.bottomWall.map(panel => `<rect x="${panel.start.x}" y="${panel.start.y}" width="${panel.size}" height="${walls.thickness}" fill="${panel.size >= 30 ? 'rgb(202, 248, 201)' : 'rgb(250, 127, 137)'}" stroke="black"/>`)).join('')}
			${(wallPanels.rightWall.map(panel => `<rect x="${panel.start.x}" y="${panel.start.y}" width="${walls.thickness}" height="${panel.size}" fill="${panel.size >= 30 ? 'rgb(202, 248, 201)' : 'rgb(250, 127, 137)'}" stroke="black"/>`)).join('')}
			${(wallPanels.leftWall.map(panel => `<rect x="${panel.start.x}" y="${panel.start.y}" width="${walls.thickness}" height="${panel.size}" fill="${panel.size >= 30 ? 'rgb(202, 248, 201)' : 'rgb(250, 127, 137)'}" stroke="black"/>`)).join('')}
			${(wallPanels.innerWalls.horizontal.map(wall => {
				return (wall.map(panel => `<rect x="${panel.start.x}" y="${panel.start.y}" width="${panel.size}" height="${walls.thickness}" fill="${panel.size >= 30 ? 'rgb(202, 248, 201)' : 'rgb(250, 127, 137)'}" stroke="black"/>`)).join('')
			})).join('')}
			${(wallPanels.innerWalls.vertical.map(wall => {
				return (wall.map(panel => `<rect x="${panel.start.x}" y="${panel.start.y}" width="${walls.thickness}" height="${panel.size}" fill="${panel.size >= 30 ? 'rgb(202, 248, 201)' : 'rgb(250, 127, 137)'}" stroke="black"/>`)).join('')
			})).join('')}
